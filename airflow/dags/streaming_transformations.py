...
with DAG(
    dag_id='transformation_streaming_dag',
    ...
    clicked_events = SparkSubmitOperator(
        task_id='clicked_events',
        conn_id="spark_default",
        application='/user/local/spark/app/clicked_events.py',
    ...
    session_data = SparkSubmitOperator(
        task_id='session_data',
        conn_id="spark_default",
        application='/user/local/spark/app/session_data.py',
        application_args=['{{ data_interval_end }}'],  # Pass the execution date as an argument
        conf={"spark.master":spark_master},
        packages="io.delta:delta-core_2.12:2.2.0,org.apache.spark:spark-sql-kafka-0-10_2.12:3.3.0,org.apache.spark:spark-avro_2.12:3.3.0",
    )
...
start_task >> clicked_events >> end_task
